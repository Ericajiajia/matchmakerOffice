...
var typewriter = function (explain, next) {
    let index = 0
    $('.computer-top-explain').text('')
    tin = setInterval(function () {
        $('.computer-top-explain')[0].insertAdjacentHTML('beforeend', explain.charAt(index))
        if (++ index == explain.length) {
            clearInterval(tin)
            index = 0
            $('.computer-top-next').text('')
            tin = setInterval(function () {
                $('.computer-top-next')[0].insertAdjacentHTML('beforeend', next.charAt(index))
...
$('.info-page-submit')[0].addEventListener('click', function () {
	...
	memberAge = parseInt($('.info-page-age input')[0].value.replace(/[^0-9]/ig,""))
